...
class StaffBookingController
{
    ...
    private const BASE_URL = "views/staff/booking";
    ...
    public static function renderScreen(Request $req, Response $res)
    {
        try {
            ...
            $batch = $batch_model->findOne(["DEPARTMENT_ID" => $department_credentials["DEPARTMENT_ID"]]);


            $batch_list = $batch_model->find(["STATUS" => "ACTIVE"]);
            $booking_list = $booking_model->find(["BATCH_ID" => $batch["BATCH_ID"]]);
